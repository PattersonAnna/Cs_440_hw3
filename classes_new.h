...
public:
    ...
    bool insert_record_into_page(Record r){  // Write a Record into your page (main memory)
        ...
        if(cur_size + r.get_size() >= 4096){  // Checking if the current Record can be entered or not
            ...
            slot_directory.push_back({
                    cur_size - r.get_size(), r.get_size()
            });
            return true;
        }
    ...
    bool read_from_data_file(istream& in) {
        ...
        if (bytes_read == 4096) {
            ...
            while(offset < 4096) {
                ...
                offset += bio_len;
                vector<string> x = {to_string(id), name, bio, to_string(manager_id)};
                records.push_back(Record(x));
                slot_directory.push_back({
                            offset, name_len + bio_len + sizeof(id) + sizeof(manager_id)
                });
            }
            return true;
        }
